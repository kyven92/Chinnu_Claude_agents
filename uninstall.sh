#!/usr/bin/env bash
# uninstall.sh — remove Chinnu-Claude-setup from ~/.claude/
# Usage (local):  ./uninstall.sh [flags]
# Usage (remote): curl -fsSL https://raw.githubusercontent.com/YOUR-ORG/Chinnu-Claude-setup/main/uninstall.sh | bash
set -euo pipefail
...
if [ -f "${SCRIPT_DIR}/bin/claude-setup" ]; then
    ...
    if ! command -v git >/dev/null 2>&1; then
        echo "ERROR: git is required for remote uninstall. Please install git and retry." >&2
        exit 1
    fi
...
exec python3 "${REPO_DIR}/bin/claude-setup" uninstall "$@"
